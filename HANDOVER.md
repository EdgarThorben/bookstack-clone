...
5. ~~Credentials/license-key table~~ — done 2026-07-14 (simulated demo, per user request). New `credentials` + `credential_reveals` tables. `src/lib/credentialCrypto.ts` implements the confirmed AES-256-GCM app-level encryption (per-record IV + authTag, key from `CREDENTIALS_ENCRYPTION_KEY` env var — read via `import.meta.env` with `process.env` fallback, matching `src/db/client.ts`'s pattern; generate one with `node -e "console.log(require('crypto').randomBytes(32).toString('base64'))"`). `src/actions/credentials.ts`: `createCredential`, `revealCredential` (writes a `credential_reveals` row on every decrypt — no per-user RBAC, matches the confirmed access model). UI: `CredentialsPanel.astro`, embedded in the asset detail page — masked by default, "Reveal" toggles a client-side astro:actions call, un-authenticated visitors see "Log in to reveal" instead of a dead-end button. Verified end-to-end: ciphertext confirmed non-plaintext in the DB, decrypt round-trips correctly, reveal count increments only on actual decrypt (not on client-side re-mask).
